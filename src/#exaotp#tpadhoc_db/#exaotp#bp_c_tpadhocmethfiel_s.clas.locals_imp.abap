CLASS LHC_/EXAOTP/I_TPADHOCMETHFIELD DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE TpAdhocMethFieldAll\_TpAdhocMethField
          ENTITIES_UPDATE FOR UPDATE TpAdhocMethField.
ENDCLASS.

CLASS LHC_/EXAOTP/I_TPADHOCMETHFIELD IMPLEMENTATION.
  METHOD AUGMENT.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE /EXAOTP/I_TpAdhocMethField\_TpAdhocMethFielText,
          text_for_existing_entity TYPE TABLE FOR CREATE /EXAOTP/I_TpAdhocMethField\_TpAdhocMethFielText,
          text_update              TYPE TABLE FOR UPDATE /EXAOTP/I_TpAdhocMethFieldText.
    ...
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK /EXAOTP/I_TpAdhocMethField\_TpAdhocMethFielText,
    ...
    LOOP AT entity-%TARGET ASSIGNING FIELD-SYMBOL(<target>).
      ...
      INSERT VALUE #( %CID_REF = <target>-%CID
                      %IS_DRAFT = <target>-%IS_DRAFT
                        ...
                        %KEY-ColumnId = <target>-%KEY-ColumnId
                      %TARGET = VALUE #( (
    ...
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF /EXAOTP/I_TpAdhocMethField_S
      ENTITY TpAdhocMethField
        CREATE BY \_TpAdhocMethFielText
        FROM text_for_new_entity
        RELATING TO entities_create BY relates_create.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF /EXAOTP/I_TpAdhocMethField_S
